...
# =========================================================
# ENTRADA RÁPIDA EM COMPETIÇÃO (APONTADOR/EQUIPE)
# =========================================================
def _partidas_colunas_leves_sql(prefixo="p"):
    """Monta SELECT leve e tolerante a colunas antigas/novas da tabela partidas."""
    colunas = _buscar_colunas_tabela("partidas")
    p = f"{prefixo}." if prefixo else ""

    def campo(nome, alias=None, padrao="NULL"):
        alias = alias or nome
        if nome in colunas:
            return f"{p}{nome} AS {alias}"
        return f"{padrao} AS {alias}"

    campos = [
        campo("id"),
        campo("competicao"),
        campo("ordem", padrao="0"),
        campo("rodada", padrao="0"),
        campo("grupo", padrao="''"),
        campo("fase", padrao="'grupos'"),
        campo("fase_partida", padrao="''"),
        campo("origem", padrao="''"),
        campo("equipe_a", padrao="''"),
        campo("equipe_b", padrao="''"),
        campo("equipe_a_operacional", padrao="''"),
        campo("equipe_b_operacional", padrao="''"),
        campo("quadra", padrao="''"),
        campo("quadra_id", padrao="NULL"),
        campo("quadra_nome", padrao="''"),
        campo("data", padrao="NULL"),
        campo("hora", padrao="NULL"),
        campo("status", padrao="'agendada'"),
        campo("status_jogo", padrao="''"),
        campo("status_operacao", padrao="'livre'"),
        campo("pre_jogo_finalizado", padrao="FALSE"),
        campo("set_atual", padrao="1"),
        campo("pontos_a", padrao="0"),
        campo("pontos_b", padrao="0"),
        campo("sets_a", padrao="0"),
        campo("sets_b", padrao="0"),
        campo("set1_a", padrao="0"),
        campo("set1_b", padrao="0"),
        campo("set2_a", padrao="0"),
        campo("set2_b", padrao="0"),
        campo("set3_a", padrao="0"),
        campo("set3_b", padrao="0"),
        campo("set4_a", padrao="0"),
        campo("set4_b", padrao="0"),
        campo("set5_a", padrao="0"),
        campo("set5_b", padrao="0"),
        campo("vencedor", padrao="''"),
        campo("tipo_encerramento", padrao="''"),
        campo("operador_login", padrao="''"),
        campo("operador_nome", padrao="''"),
    ]
    return ",\n                    ".join(campos)


def buscar_competicao_resumo(competicao):
    """Busca somente os campos usados para abrir painel rápido."""
    if not competicao:
        return None
    try:
        colunas = _buscar_colunas_tabela("competicoes")
        campos_base = ["nome"]
        for campo in ["sets_tipo", "modo_operacao", "pontos_set", "pontos_tiebreak", "tem_tiebreak", "diferenca_minima"]:
            if campo in colunas:
                campos_base.append(campo)
        with conectar() as conn:
            with conn.cursor() as cur:
                cur.execute(f"""
                    SELECT {', '.join(campos_base)}
                    FROM competicoes
                    WHERE nome = %s
                    LIMIT 1
                """, (competicao,))
        ...
        print("AVISO buscar_competicao_resumo:", repr(e))
        return {"nome": competicao, "sets_tipo": "melhor_de_3", "modo_operacao": "simples"}


def listar_partidas_apontador_resumo(competicao):
    """Lista leve para a entrada do apontador na competição.

    Não traz escudos, atletas, papeletas ou eventos detalhados. O total de eventos
    vem agregado em uma subquery única para evitar uma consulta por partida.
    """
    if not competicao:
        return []
    campos = _partidas_colunas_leves_sql("p")
    try:
        with conectar() as conn:
            with conn.cursor() as cur:
                cur.execute(f"""
                    SELECT
                        {campos},
                        COALESCE(ev.total, 0) AS eventos_total,
        ...
        print("AVISO listar_partidas_apontador_resumo, fallback listar_partidas:", repr(e))
        return listar_partidas(competicao)


def listar_partidas_equipe_resumo(competicao, equipe_nome):
    """Lista leve para dashboard da equipe: só partidas daquela equipe."""
    ...
    try:
        with conectar() as conn:
            with conn.cursor() as cur:
                cur.execute(f"""
                    ...
                    WHERE p.competicao = %s
                      AND (
                          LOWER(TRIM(COALESCE(p.equipe_a, ''))) = LOWER(TRIM(%s))
                       OR LOWER(TRIM(COALESCE(p.equipe_b, ''))) = LOWER(TRIM(%s))
                       OR LOWER(TRIM(COALESCE(p.equipe_a_operacional, ''))) = LOWER(TRIM(%s))
                       OR LOWER(TRIM(COALESCE(p.equipe_b_operacional, ''))) = LOWER(TRIM(%s))
                      )
                    ORDER BY COALESCE(p.rodada, 999999), COALESCE(p.ordem, 999999), p.id
                """, (competicao, equipe_nome, equipe_nome, equipe_nome, equipe_nome))
                linhas = cur.fetchall() or []
                for linha in linhas:
                    linha["minha_partida"] = True
                    try:
                        if linha.get("quadra_id") and linha.get("quadra_nome_cadastro"):
                            linha["quadra_nome"] = formatar_quadra_exibicao({
                                "nome": linha.get("quadra_nome_cadastro"),
        ...
        print("AVISO listar_partidas_equipe_resumo, fallback listar_partidas:", repr(e))
        return [p for p in (listar_partidas(competicao) or []) if (str(p.get("equipe_a") or "").strip().lower() == equipe_nome.lower() or str(p.get("equipe_b") or "").strip().lower() == equipe_nome.lower())]


def contar_atletas_equipe_status(competicao, equipe_nome):
    """Conta atletas por status sem transferir a lista inteira para dashboard."""
    resultado = {"total": 0, "aprovados": 0, "pendentes": 0, "reprovados": 0}
    competicao = (competicao or "").strip()
    equipe_nome = (equipe_nome or "").strip()
    if not competicao or not equipe_nome:
        return resultado
    try:
        colunas = _buscar_colunas_tabela("atletas")
        if "equipe" not in colunas:
            return resultado
        filtro_comp = "AND competicao = %s" if "competicao" in colunas else ""
        params = [equipe_nome]
        if filtro_comp:
        ...
        with conectar() as conn:
            with conn.cursor() as cur:
                cur.execute(f"""
                    SELECT
                        COUNT(*) AS total,
                        SUM(CASE WHEN LOWER(COALESCE(status, '')) = 'aprovado' THEN 1 ELSE 0 END) AS aprovados,
                        SUM(CASE WHEN LOWER(COALESCE(status, '')) = 'reprovado' THEN 1 ELSE 0 END) AS reprovados,
                        SUM(CASE WHEN LOWER(COALESCE(status, '')) IN ('', 'pendente', 'aguardando', 'em análise', 'em analise', 'em_analise') THEN 1 ELSE 0 END) AS pendentes
                    FROM atletas
                    WHERE LOWER(TRIM(equipe)) = LOWER(TRIM(%s))
                    {filtro_comp}
                """, tuple(params))
                row = cur.fetchone() or {}
                resultado["total"] = int(row.get("total") or 0)
                resultado["aprovados"] = int(row.get("aprovados") or 0)
                resultado["pendentes"] = int(row.get("pendentes") or 0)
                resultado["reprovados"] = int(row.get("reprovados") or 0)
                return resultado
    except Exception as e:
        print("AVISO contar_atletas_equipe_status:", repr(e))
        return resultado
